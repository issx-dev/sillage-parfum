    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm tabular-nums text-warm-500">Catálogo con búsqueda instantánea</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <AutoSubmitForm className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
            <label htmlFor="busqueda-producto" className="sr-only">
              Buscar por nombre, marca o inspiración
            </label>
            <Input
              id="busqueda-producto"
              name="q"
              type="search"
              placeholder="Buscar por nombre, marca, inspiración…"
              defaultValue={search}
        ...
        <CardContent>
          ...
          <Suspense key={`${search}::${page}`} fallback={<ResultadosSkeleton />}>
            <CatalogoResultados search={search} page={page} />
          </Suspense>
        </CardContent>
      </Card>
    </div>
...
function ResultadosSkeleton() {
  return (
    <div className="space-y-2" aria-busy="true" aria-label="Filtrando productos">
      {[0, 1, 2, 3].map((i) => (
...
async function CatalogoResultados({ search, page }: { search: string; page: number }) {
        ...
        <TableBody>
          {rows.map((p) => (
            <TableRow key={p.product_id}>
                ...
                <span className="flex items-center gap-3">
                  ...
                  <span>
                    ...
                    <span className="block text-xs tabular-nums text-warm-500">
                      ...
                      {p.inspiration ? ` · Inspirado en ${p.inspiration}` : ""}
                    </span>
                  </span>
                </span>
              </TableCell>
              ...
              <TableCell className="text-right tabular-nums">
                {p.total_stock === 0 ? <Badge variant="danger">0</Badge> : String(p.total_stock)}
              </TableCell>
